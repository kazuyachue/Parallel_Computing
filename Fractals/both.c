...
void init()
{
   ...
   for( x = 0 ; x < N ; x++ )
   {
      #pragma omp parallel for private(y, step)
      for( y = 0 ; y < N ; y++ )
      {
	double x0 = xmin + x * (xmax - xmin) / N;
	double y0 = ymin + y * (ymax - ymin) / N; 
	double zR = 0.0;
	double zI = 0.0;
	step = 0;
...
void displayfunc()
{
   ...
   for( x = 0 ; x < N ; x++ ) //julia
   {
      ...
      { 
         ...
         {
            glColor3f( 0.5 * t2[x][y] , 0.5 * t2[x][y], 0.5 * t2[x][y]) ; // purple
         }
         //
         glBegin(GL_POINTS);
         glVertex2f(x + N,y);
...
{
   glViewport(0,0,2*(GLsizei)N,(GLsizei)N);
   glMatrixMode(GL_PROJECTION);
   glLoadIdentity();
   gluOrtho2D(0.0,2.0*N,0.0,1.0*N); // always a square
...
void julia(int xscr, int yscr)
{
   ...
   for( x = 0 ; x < N ; x++ )
   {
  	...
  	double x0 = xmin + xscr * (xmax - xmin) / N;
        ...
        if (step == maxsteps) t2[x][y] = -1.0;
        else t2[x][y] = 1 - pow ( 1.0 - (1.0 * step)/(1.0 * maxsteps), 3);
      }
   }
   displayfunc();
}
